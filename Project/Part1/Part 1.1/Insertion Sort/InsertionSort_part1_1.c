...
int main()
    ...
    end_t = clock();
Ordered_maximum_sort(values,1405);
    printMeasurments2(values); //function in order to print the data struct.
...
void Ordered_maximum_sort(measurements values[],int size)
{
    ...
    char max_date[50];
    float number=values[0].temp;
    int start=0;
    for(int i=1; i<=size; i++)
    {
        if(number!=values[i].temp || i==size)
        {
